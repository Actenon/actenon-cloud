...
| Auth | Development-signed bearer tokens with a bootstrap admin flow, persisted roles, tenant memberships, and permission enforcement. An `external_managed_bearer` stub now fails intentionally until managed bearer verification is integrated. | Enough access control exists for a controlled pilot with explicit operator ownership and non-default secrets. | No enterprise SSO, no federation, no production workload identity, and no mature operator lifecycle controls. |
| Signing | Proof issuance works with the current development-local HS256 HMAC signer. An `external_managed` signing stub now fails intentionally until a real managed signer is integrated. | The repo can issue pilot proofs and record signing operations for controlled evaluation. | No KMS-backed or HSM-backed signing, no production trust anchor management, and no production signing ceremony. |
| Evidence storage | Evidence metadata is persisted and uploads are written to a filesystem-backed path checked by readiness. | A managed pilot can use mounted persistent writable storage with explicit backup handling. | No native object-store upload path, no storage-tier isolation story, and no production evidence durability posture. |
| Observability | Structured application logs, request IDs, and live or ready health endpoints are implemented. The runtime reports metrics and tracing as placeholders only. | Operators can monitor a pilot through health endpoints and centralized log collection. | No metrics exporter, no tracing backend, no alerting pipeline, and no exercised operational dashboards or paging flow. |
...
- replace development bearer auth with enterprise operator identity and production workload identity
- replace development-local signing with managed signing infrastructure
- add a production-grade evidence storage adapter and durability story
- add metrics, tracing, alerting, dashboards, and practiced incident response
- automate deployment, rollback, and restore workflows
- harden tenant isolation and hosting controls
